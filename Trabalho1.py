...
def sjf(entradas,preempcao,verbose):
        ...
        if(preempcao):
                ...
                while tempo_atual>=0:
                        if(len(fila)>0 and fila[0][1]==tempo_atual):
                                pronto.append(fila.pop(0)) #Processo chegou no escalonador
                                if(pronto[0][2]>0):#checa se o processo ja foi executado
                                        pronto=sorted(pronto, key=lambda tup: tup[0])
                        else:
                                if(len(pronto)==0):
                                        if(len(fila)==0):
                                ...
                                if(pronto[0][2]>0): #checa se o processo ja foi executado
                                        resposta.append(tempo_atual-int(pronto[0][1])) 
                                        pronto[0][2]=pronto[0][2]*-1 #Inverte valor do tempo de duracao total para indicar que ja foi executado
                                tempo_atual+=1
                                pronto[0][0]-=1
                                if(pronto[0][0]==0):
                                        retorno.append(tempo_atual-int(pronto[0][1]))
...
retorno,resposta,espera=rr(entradas,2,False)
if(printar!=0):
